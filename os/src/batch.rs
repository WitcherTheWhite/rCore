...
static KENAL_STACK: KernalStack = KernalStack {
...
impl AppManager {
    fn print_app_info(&self) {
        println!("[kernal] num_app = {}", self.num_app);
...
pub fn run_next_app() -> ! {
    ...
    unsafe {
        __restore(KENAL_STACK.push_context(TrapContext::app_init_context(
            APP_BASE_ADDRESS,
            USER_STACK.get_sp(),
        )) as *const _ as usize)
    }
